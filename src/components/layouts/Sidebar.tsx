    ...
    <React.Fragment>
      <nav x-show="currentSidebarTab == 'linksTab'" aria-label="Main" className="flex flex-col h-full">
        ...
        <div className="bg-white rounded-md p-3 drop-shadow-md">
          <div className="pl-2 text-zinc-400 pt-2 text-md">
            Bạn có
          </div>
          <div className="flex flex-wrap">
            <div className="w-28 p-2 text-center text-6xl font-bold text-rose-600">
              { padWithLeadingZeros(numtodayTasks || 0 , 2) }</div>
        ...
        <div className="flex flex-wrap justify-between items-center pt-6 py-4 max-w-screen-xl">
          <button type="button" className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br w-[144px] focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm py-3 text-center mb-2"
        ...
        <div className="text-center flex flex-row items-center justify-evenly w-[350] flex-wrap">
          <Link to="/Todo" className="" onClick={ () => changeState(0) }>
            <Button className={ state === 0 ? "bg-white w-32 h-32 uppercase font-bold border-none border-white shadow-md text-blue-600 flex flex-col items-center" : "bg-slate-100 w-36 h-36 uppercase font-bold border-none shadow border-slate-700 text-slate-500 flex flex-col items-center" }>
              <FaTasks className="text-4xl mt-8 mb-2" />
              <span> Việc cần làm </span>
            </Button>
          </Link>
          <Link to="/" className="ml-2" onClick={ () => changeState(1) }>
            <Button className={ state === 1 ? "bg-white w-32 h-32 uppercase font-bold border-none border-white shadow-md text-blue-600 flex flex-col items-center" : "bg-slate-100 w-36 h-36 uppercase font-bold border-none shadow border-slate-700 text-slate-500 flex flex-col items-center" }>
